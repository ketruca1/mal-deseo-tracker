  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2.5">
        ...
        {isComplete ? (
          <Trophy className="h-[16px] w-[16px] text-emerald-500 shrink-0 mt-0.5" strokeWidth={1.5} />
        ) : pct >= 75 ? (
          <Zap className="h-[16px] w-[16px] text-amber-500 shrink-0 mt-0.5" strokeWidth={1.5} />
        ) : (
          <Target className="h-[16px] w-[16px] text-[#48484a] shrink-0 mt-0.5" strokeWidth={1.5} />
        )}
      </div>
        ...
        <div>
          {isEditing ? (
            ...
            <button onClick={onStartEdit} className="flex items-baseline gap-2 group">
              <span className="text-[24px] font-bold text-white tracking-[-0.03em] leading-none tabular-nums group-hover:text-[#D6001C] transition-colors duration-200">
                {kpi.current.toLocaleString()}
              </span>
              {kpi.unit && (
                <span className="text-[12px] text-[#6e6e73]">{kpi.unit}</span>
              )}
              <Edit3 className="h-[12px] w-[12px] text-[#48484a] group-hover:text-[#6e6e73] transition-colors" />
            </button>
          )}
        </div>
      ...
      {/* Progress bar */}
      <div className="h-[3px] rounded-full bg-white/[0.04] mb-3">
        <motion.div
          initial={{ width: 0 }}
          animate={{ width: `${pct}%` }}
      ...
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-[#48484a]">
          Meta: <span className="text-[#6e6e73] font-medium">{kpi.target.toLocaleString()} {kpi.unit}</span>
        ...
        {kpi.platform && (
          <span className={`text-[10px] font-medium px-2 py-[3px] rounded-full ${
            kpi.platform === "tiktok"
              ? "bg-[#D6001C]/[0.08] text-[#ff6b7a]"
              : "bg-[#ec4899]/[0.08] text-[#f9a8d4]"
          }`}>
    ...
    <div className="space-y-4">
      ...
      <div className="glass-accent p-4 flex items-center gap-4">
        ...
        <div className="flex-1 space-y-2.5">
          <div>
            <p className="text-[13px] font-semibold text-white tracking-[-0.01em]">Progreso General</p>
            <p className="text-[11px] text-[#6e6e73] mt-0.5">
              {completed} de {kpis.length} objetivos
            </p>
          </div>
          <div className="flex gap-2">
            <div className="flex-1 py-1.5 px-2.5 rounded-xl bg-white/[0.03] border border-white/[0.04]">
              <p className="text-[16px] font-bold text-emerald-500 tracking-[-0.02em]">{completed}</p>
              <p className="text-[9px] text-[#48484a] uppercase tracking-[0.06em] font-medium mt-[-1px]">Logrados</p>
            </div>
            <div className="flex-1 py-1.5 px-2.5 rounded-xl bg-white/[0.03] border border-white/[0.04]">
              <p className="text-[16px] font-bold text-amber-500 tracking-[-0.02em]">{onTrack}</p>
              <p className="text-[9px] text-[#48484a] uppercase tracking-[0.06em] font-medium mt-[-1px]">En camino</p>
            </div>
            <div className="flex-1 py-1.5 px-2.5 rounded-xl bg-white/[0.03] border border-white/[0.04]">
              <p className="text-[16px] font-bold text-[#6e6e73] tracking-[-0.02em]">
                {kpis.length - completed - onTrack}
              </p>
              <p className="text-[10px] text-[#48484a] uppercase tracking-[0.06em] font-medium mt-[-1px]">En riesgo</p>
            </div>
          </div>
        </div>
      </div>
        ...
        return (
          <div key={category}>
            <div className="flex items-center gap-2.5 mb-3 px-0.5">
              <div className={`p-1.5 rounded-[10px] ${config.bg}`}>
                <CatIcon className={`h-[14px] w-[14px] ${config.color}`} strokeWidth={1.5} />
              </div>
              <span className="text-[11px] font-semibold text-[#6e6e73] uppercase tracking-[0.08em]">
                {config.label}
              </span>
              <span className="text-[10px] text-[#48484a] ml-auto tabular-nums">

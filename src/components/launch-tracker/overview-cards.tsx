...
function HeroStat({ label, value, subtitle, accent }: { label: string; value: string; subtitle?: string; accent?: boolean }) {
  return (
    <div className={`p-4 ${accent ? "glass-accent" : "glass"}`}>
      <p className="text-[10px] text-[#6e6e73] uppercase tracking-[0.08em] font-medium">{label}</p>
      <p className="text-[26px] font-bold text-white mt-1 tracking-[-0.03em] leading-none text-glow">{value}</p>
      {subtitle && <p className="text-[11px] text-[#48484a] mt-1.5">{subtitle}</p>}
    </div>
  );
}

function PlatformSection({ platform, data, accentColor }: { platform: "TikTok" | "Instagram"; data: OverviewData["tiktok"] | OverviewData["instagram"]; accentColor: string }) {
    ...
    <div className="glass p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-[6px] h-[6px] rounded-full" style={{ backgroundColor: accentColor, boxShadow: `0 0 12px ${accentColor}50` }} />
          <span className="text-[13px] font-semibold text-white tracking-[-0.01em]">{platform}</span>
        </div>
        <span className="text-[10px] text-[#48484a] font-medium">{data.daysTracked} días</span>
      </div>
      <div className="flex items-end justify-between">
        <div>
          <p className="text-[10px] text-[#48484a] uppercase tracking-[0.08em] font-medium">Visualizaciones</p>
          <p className="text-[30px] font-bold text-white mt-1 tracking-[-0.03em] leading-none text-glow">{formatNumber(data.totalViews)}</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-[#48484a] uppercase tracking-[0.08em] font-medium">Engagement</p>
        ...
        {metrics.map((m) => {
          ...
          return (
            <div key={m.label} className="text-center p-2 rounded-[14px] bg-white/[0.015] border border-white/[0.03]">
              ...
              <p className="text-[14px] font-semibold text-white mt-1.5 tracking-[-0.01em] tabular-nums">{formatNumber(m.value)}</p>
      ...
      {"profileViews" in data && data.profileViews > 0 && (
        <div className="flex items-center justify-between pt-3.5 border-t divider-red">
          <div className="flex items-center gap-2"><Users className="h-[12px] w-[12px] text-[#48484a]" strokeWidth={1.5} /><span className="text-[12px] text-[#6e6e73]">Visitas al perfil</span></div>
          <span className="text-[14px] font-semibold text-white tracking-[-0.01em]">{data.profileViews.toLocaleString()}</span>
        </div>
      )}
    </div>
  );
...
export default function OverviewCards({ data }: { data: OverviewData }) {
  return (
    <div className="space-y-3">
      <p className="text-[12px] font-semibold text-[#6e6e73] uppercase tracking-[0.08em]">Resumen General</p>
      <div className="grid grid-cols-2 gap-3">
        <HeroStat label="Views Totales" value={formatNumber(data.combined.totalViews)} subtitle="TikTok + Instagram" accent />
        <HeroStat label="Interacciones" value={formatNumber(data.combined.totalInteractions)} subtitle="Likes · Comments · Shares" />
      </div>
      <div className="space-y-3">
